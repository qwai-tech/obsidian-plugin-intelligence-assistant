...
- ⚠️ **代码文件不得含中文**:所有 `.ts` 代码(含注释)一律用英文。本文档(散文说明)用中文,但代码块内一律英文。
...
在 `tool-registry.test.ts` 顶部 `@/types/common/tools` 那行 import 追加 `AgentToolAccess`,即把那一行改为:
