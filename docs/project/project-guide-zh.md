...
- `src/infrastructure`：LLM 提供商、CLI SDK 桥接、持久化、RAG/向量库实现
...
- MCP、OpenAPI、CLI agent、RAG 都会引入异步初始化，启动顺序相关 bug 比较常见。
