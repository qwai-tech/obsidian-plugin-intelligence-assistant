...
export class ProviderRepository {
	...
	constructor(private readonly app: App, private readonly filePath = LLM_PROVIDERS_PATH) {}

	async initialize(): Promise<void> {
		if (this.initialized) return;
		await ensureFolderExists(this.app.vault.adapter, DATA_FOLDER);
		if (!(await this.app.vault.adapter.exists(this.filePath))) {
			await this.app.vault.adapter.write(this.filePath, JSON.stringify({ version: 1, providers: [] }, null, 2));
		}
