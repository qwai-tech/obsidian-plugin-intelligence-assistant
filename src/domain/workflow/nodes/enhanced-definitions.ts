...
export const parseCSVNode: NodeDef = {
  ...
  async execute(inputs, config, context) {
    ...
    try {
      const csvText = inputs[0]?.json?.text || inputs[0]?.json?.csv || '';
      if (!csvText) {
        ...
        case 'extract':
          ...
          for (const part of pathParts) {
            if (currentValue === null || currentValue === undefined) {
              throw new Error(`Path does not exist: ${propertyName}`);
            }
    ...
    try {
      ...
      for (const condition of conditions) {
        try {
          ...
          context.log(`Condition evaluation failed for "${condition.name}": ${error.message}`);
        }
      }
      ...
      throw new Error(`Secure switch evaluation failed: ${error.message}`);
    }
  },
};
...
export const readFileNode: NodeDef = {
  ...
  async execute(inputs, config, context) {
    ...
    const filePath = inputs[0]?.json?.path || inputs[0]?.json?.text || inputs[0]?.json?.filePath || '';
    ...
    try {
      ...
      const file = context.services.vault.getFileByPath(filePath);
      if (!file) {
        if (fallbackContent) {
          return [{
            json: {
      ...
      if (readAsBinary) {
        const arrayBuffer = await context.services.vault.readBinary(file);
        content = arrayBuffer;
      } else {
        content = await context.services.vault.read(file);
      }
      ...
      if (parseJSON && typeof content === 'string') {
        try {
          ...
          context.log(`Warning: Could not parse file as JSON: ${parseError.message}`);
        }
      }
      
      // Get file stats
      const stat = await context.services.vault.getFileStat(file);
...
export const rssFeedReaderNode: NodeDef = {
  ...
  async execute(inputs, config, context) {
    ...
    try {
      ...
      const feedContent = typeof response.data === 'string' ? response.data : response.text || '';
      
      // Simple RSS/Atom parsing - in a real implementation you'd use a proper parser
      const feedData = this.parseFeed(feedContent, feedUrl);
        ...
        for (const item of feedData.items) {
          if (item.link && !item.fullContent) {
            try {
              ...
              const htmlContent = typeof contentResponse.data === 'string' ? contentResponse.data : contentResponse.text || '';
              const textContent = htmlContent.replace(/<[^>]*>/g, '').substring(0, 1000);
              item.fullContent = textContent;
            } catch (error) {
              context.log(`Warning: Failed to fetch full content for ${item.link}: ${error.message}`);
            }
          }
        }
      }
      ...
      throw new Error(`Failed to read RSS feed ${feedUrl}: ${error.message}`);
    }
  },
